...
-- --------------------------------------------------
-- Date Created: 05/20/2015 15:14:28
-- Generated from EDMX file: C:\Users\mobile4\Desktop\NO\BusinessTierBudgetUp\BusinessTierBudgetUp\DatabaseClasses\Model1.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_DurationTypeProject]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Projects] DROP CONSTRAINT [FK_DurationTypeProject];
GO
...
GO
IF OBJECT_ID(N'[dbo].[FK_TravelCarExpense]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[CarExpenses] DROP CONSTRAINT [FK_TravelCarExpense];
GO
...
GO
IF OBJECT_ID(N'[dbo].[CarExpenses]', 'U') IS NOT NULL
    DROP TABLE [dbo].[CarExpenses];
GO
...
GO
IF OBJECT_ID(N'[dbo].[DurationTypes]', 'U') IS NOT NULL
    DROP TABLE [dbo].[DurationTypes];
GO
...
-- Creating table 'CarExpenses'
CREATE TABLE [dbo].[CarExpenses] (
    [Id] int IDENTITY(1,1) NOT NULL,
    [UP_Fleet] bit  NOT NULL,
    [Amount] float  NOT NULL,
    [Travel_Id] int  NOT NULL
);
GO
...
-- Creating primary key on [Id] in table 'CarExpenses'
ALTER TABLE [dbo].[CarExpenses]
ADD CONSTRAINT [PK_CarExpenses]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO
...
-- Creating primary key on [Id] in table 'DurationTypes'
ALTER TABLE [dbo].[DurationTypes]
ADD CONSTRAINT [PK_DurationTypes]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO
...
-- Creating foreign key on [DurationTypeId] in table 'Projects'
ALTER TABLE [dbo].[Projects]
ADD CONSTRAINT [FK_DurationTypeProject]
    FOREIGN KEY ([DurationTypeId])
    REFERENCES [dbo].[DurationTypes]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_DurationTypeProject'
CREATE INDEX [IX_FK_DurationTypeProject]
ON [dbo].[Projects]
    ([DurationTypeId]);
GO
...
-- Creating foreign key on [Travel_Id] in table 'CarExpenses'
ALTER TABLE [dbo].[CarExpenses]
ADD CONSTRAINT [FK_TravelCarExpense]
    FOREIGN KEY ([Travel_Id])
    REFERENCES [dbo].[Travels]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;

-- Creating non-clustered index for FOREIGN KEY 'FK_TravelCarExpense'
CREATE INDEX [IX_FK_TravelCarExpense]
ON [dbo].[CarExpenses]
    ([Travel_Id]);
GO
